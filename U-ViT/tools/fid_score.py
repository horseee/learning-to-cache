...
from .read_npz import open_npz_array
...
def compute_statistics_of_images_in_npz(path, model, batch_size, dims, device, num_workers=8):
    model.eval()   
    pred_arr = np.empty((50000, dims))
    start_idx = 0

    with open_npz_array(path, "arr_0") as reader:
        for samples in tqdm(reader.read_batches(batch_size), total=50000 // batch_size):
            samples = np.array(samples)
            samples = torch.from_numpy(samples.transpose(0, 3, 1, 2)).contiguous().to(device)
            samples = samples.div(255).float()
...
def calculate_fid_given_paths(paths, device=None, batch_size=50, dims=2048, num_workers=8):
    ...
    if paths[1].endswith('.npz'):
        m2, s2 = compute_statistics_of_images_in_npz(paths[1], model, batch_size,
                                        dims, device, num_workers)
    else:
