...
def get_nnet(name, **kwargs):
    if name == 'uvit':
        ...
        return UViT(**kwargs)
    elif name == 'uvit_timecache':
        from libs.uvit_timecache import UViT
        return UViT(**kwargs)
    elif name == 'uvit_router':
        from libs.uvit_router import UViT
        return UViT(**kwargs)
    elif name == 'uvit_dynamic':
        from libs.uvit_dynamic import UViT
        return UViT(**kwargs)
    elif name == 'uvit_manual':
        from libs.uvit_manual import UViT
        return UViT(**kwargs)
    elif name == 'uvit_deepcache':
        from libs.uvit_deepcache import UViT
        return UViT(**kwargs)
    elif name == 'uvit_fasterdiffusion':
        from libs.uvit_fasterdiffusion import UViT
        return UViT(**kwargs)
    elif name == 'uvit_analysis':
        from libs.uvit_analysis import UViT
        return UViT(**kwargs)
    elif name == 'uvit_ranklayer':
        from libs.uvit_ranklayer import UViT
        return UViT(**kwargs)
    else:
...
class TrainState(object):
    ...
    def save(self, path):
        ...
        for key, val in self.__dict__.items():
            if key != 'step' and 'ema' not in key and val is not None:
                if key == 'nnet':
                    torch.save(val.routers.state_dict(), os.path.join(path, f'{key}.pth'))
                else:
